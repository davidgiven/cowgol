...
static struct spec_aout S[]= {
	...
	{ 0x1ae00, IS_V1 },	/* bin/strip from s2 tape (gray)*/
...
int special_magic(u_int16_t *cptr)
{
    ...
    for (i=0; S[i].cksum!=0; i++) if (S[i].cksum==cksum) {
      ...
      return(S[i].environment);
   				...
   				/* None, return 0 */
    if (default_aout_type == IS_UNKNOWN)
   	printf("Apout - unknown magic in header: 0x%x\n", cksum);
    return(default_aout_type);
}
