...
void run() {
    ...
    if (trap_debug) {
	(void)fprintf(dbg_file, "Just starting to run pid %d\n",(int)getpid());
	(void)fprintf(dbg_file, "Regs are           ");
	for (i=0;i<=PC;i++) (void)fprintf(dbg_file, "%06o ",regs[i]);
	(void)fprintf(dbg_file, "\n");
    }
#endif
	...
	if (inst_debug) {
	   ...
	   (void)fprintf(dbg_file, "%06o %06o %4s ", regs[7], ir, name);
	   (void)fprintf(dbg_file, "%06o %06o %06o %06o %06o %06o %06o   ",
		regs[0], regs[1], regs[2], regs[3],
		regs[4], regs[5], regs[6]);
	   (void)fprintf(dbg_file, "NZVC %d%d%d%d\n",CC_N,CC_Z,CC_V,CC_C);
	   fflush(dbg_file);
	}
	regs[PC] += 2; itab[ir >> 6] ();
...
{
    (void)fprintf(stderr, "Apout - pid %d bus error at PC 0%06o\n",
					(int)getpid(), regs[PC]);
    (void)printf("%06o  ", ir);
    (void)printf("%o %o %o %o %o %o %o %o  ",
	 regs[0], regs[1], regs[2], regs[3],
	 regs[4], regs[5], regs[6], regs[7]);
    (void)printf("NZVC are %d%d%d%d\n",CC_N,CC_Z,CC_V,CC_C);
    exit(EXIT_FAILURE);
}

void seg_fault() {
    (void)fprintf(stderr, "Apout - pid %d segmentation fault at PC 0%06o\n",
					(int)getpid(), regs[PC]);
    (void)printf("%06o  ", ir);
    (void)printf("%o %o %o %o %o %o %o %o  ",
	 regs[0], regs[1], regs[2], regs[3],
...
void waiti() {
    (void)fprintf(stderr, "Apout - pid %d waiti instruction at PC 0%o\n",
					(int)getpid(), regs[PC]);
    exit(EXIT_FAILURE);
}
void halt() {
    (void)fprintf(stderr, "Apout - pid %d halt instruction at PC 0%o\n",
					(int)getpid(), regs[PC]);
    exit(EXIT_FAILURE);
}
void iot() {
    (void)fprintf(stderr, "Apout - pid %d iot instruction at PC 0%o\n",
					(int)getpid(), regs[PC]);
    exit(EXIT_FAILURE);
}
void emt() {
    (void)fprintf(stderr, "Apout - pid %d emt instruction at PC 0%o\n",
					(int)getpid(), regs[PC]);
    exit(EXIT_FAILURE);
}
void bpt() {
    (void)fprintf(stderr, "Apout - pid %d bpt instruction at PC 0%o\n",
					(int)getpid(), regs[PC]);
    exit(EXIT_FAILURE);
}
void illegal() {
    (void)fprintf(stderr, "Apout - pid %d illegal instruction %o at PC 0%o\n",
					(int)getpid(),ir, regs[PC]);
    exit(EXIT_FAILURE);
}
void not_impl() {
    (void)fprintf(stderr, "Apout - pid %d unimplemented instruction at PC 0%o\n",
					(int)getpid(), regs[PC]);
    exit(EXIT_FAILURE);
}
void mark() {
    (void)fprintf(stderr, "Apout - pid %d mark instruction at PC 0%o\n",
					(int)getpid(), regs[PC]);
    exit(EXIT_FAILURE);
}
void mfpd() {
    (void)fprintf(stderr, "Apout - pid %d mfpd instruction at PC 0%o\n",
					(int)getpid(), regs[PC]);
    exit(EXIT_FAILURE);
}
void mtpd() {
    (void)fprintf(stderr, "Apout - pid %d mtpd instruction at PC 0%o\n",
					(int)getpid(), regs[PC]);
    exit(EXIT_FAILURE);
}
void trap() {
    (void)fprintf(stderr, "Apout - pid %d trap instruction at PC 0%o\n",
					(int)getpid(), regs[PC]);
    exit(EXIT_FAILURE);
}
void bad_FP_reg() {
    (void)fprintf(stderr, "Apout - pid %d bad FP register used at PC 0%o\n",
					(int)getpid(), regs[PC]);
    exit(EXIT_FAILURE);
}
