...
int stderr_flag;                /* Dump debugging output to stderr */
FILE *dbg_file = NULL;		/* Debugging output file */
char *progname = NULL;		/* The program's name - used in debugging */
int default_aout_type = IS_UNKNOWN; /* Default a.out type */
#endif
...
main(int argc, char **argv)
{
    ...
    while (1) {
      ...
      if (!strcmp(argv[1], "-stderr"))
	        { stderr_flag = 1; argc--; argv++; continue; }
      if (!strcmp(argv[1], "-type")) { 
      		argc--; argv++;
      		if (!argv[1]) {
      			fprintf(stderr, "Apout - -type requires an argument\n");
      			continue;
      		}
      		default_aout_type = xlate_aout_type(argv[1]);
      		argc--; argv++;
      		continue;
      }
      break;
...
int
xlate_aout_type(char *s)
{
  struct typetab {
    char *name;
    int  type;
  };
  static struct typetab typetab[] = {
    "v1", IS_V1,
    "v2", IS_V2,
    "v3", IS_V3,
    "v4", IS_V4,
  ...
  for (tp = typetab; tp->name; tp++)
    if (strcmp(s, tp->name) == 0)
      return tp->type;
  fprintf(stderr, "Apout - not a type: %s\n", s);
  return IS_UNKNOWN;
}
