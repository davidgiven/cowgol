...
int load_aout_header(FILE * zin, struct exec * E)
{
  ...
  cptr = (char *) &(E->a_text);
  if (fread(cptr, 1, sizeof(struct exec) - sizeof(u_int16_t), zin) < 16 - sizeof(u_int16_t))
	  return (-1);
 ...
 */
int load_script(const char *file, FILE * zin, int want_env)
{
#define SCRIPT_LINESIZE 512	/* Max size of 1st line in script */
  ...
  free(script_line);
  return (load_a_out(file, want_env));
}
 ...
 */
int load_a_out(const char *file, int want_env)
...
#define V12_MEMBASE 16384	/* Offset for V1/V2 binaries load */
  ...
  if (e.a_magic == ANY_SCRIPT) {	/* Shell script, run that */
    return (load_script(file, zin, want_env));
  }
#ifndef EMU211
  if (Binary == IS_211BSD) {
  ...
  for (size = e.a_text; size;) {
    ...
    if (i == -1) { (void) fclose(zin); return (i); }
	    ...
	    fprintf(stderr, "Apout - file %s too short (missing %d bytes)\n",
		    file, size);
	    return -1;
    }
    size -= i;
