...
int main(int argc, char* argv[])
{
    ...
    for (i = 1; i < argc; i++)
        if (strcmp(argv[i], "-I") == 0)
            ...
            fatal(
                "usage: %s [-T | -I | -p prefix | -maxcost=ddd ]... [ [ input ] output \n",
                argv[0]);
        else if (infp == NULL)
        {
            if (strcmp(argv[i], "-") == 0)
    ...
    for (r = p->rules; r; r = r->next)
    {
        switch (p->arity)
        {
            ...
            case 2:
                ...
                {
                    print("%2if (%1/* %R */\n", r);
                    emittest(r->pattern->left, "l", r->pattern->right->nterms ? " && " : " ");
...
static void emitrecord(char* pre, Rule r, int cost)
{
    print("%sif (", pre);
    if (Tflag)
        print("%Ptrace(%Pnp, %d, c + %d, p->cost[%P%S_NT]), ", r->ern, cost, r->lhs);
    print(
        "c + %d < p->cost[%P%S_NT]) {\n"
...
static void emitstate(Term terms, Nonterm start, int ntnumber)
{
    ...
    print(
        "STATE_TYPE %Pstate(int op, STATE_TYPE left, STATE_TYPE right) {\n%1int c;\n"
...
static void emittest(Tree t, char* v, char* suffix)
{
    ...
    if (p->kind == TERM)
    {
        ...
        if (t->left)
            emittest(
                t->left, stringf("%s->left", v), t->right && t->right->nterms ? " && " : suffix);
        if (t->right)
