...
void file_set_drive(int drive, const char* path)
{
	if ((drive < 0) || (drive >= NUM_DRIVES))
		exit_error("bad drive letter");
		
	if (drives[drive] != -1)
		close(drives[drive]);
	drives[drive] = open(path, O_RDONLY);
	if (drives[drive] == -1)
		exit_error("could not open '%s': %s", path, strerror(errno));

	struct stat st;
	fstat(drives[drive], &st);
	if (!S_ISDIR(st.st_mode))
		exit_error("could not open '%s': not a directory", path);
	logf("[drive %c now pointing at %s (fd %d)]\n", drive+'A', path, drives[drive]);
}

static void bump(struct file* f)
{
	// logf("[bumping file %ld to front]\n", f-files);
...
static struct file* find_file(cpm_filename_t* filename)
{
	...
	for (;;)
	{
		...
		{
			logf("[allocating file %ld for '%.11s']\n", f-files, filename->bytes);
			bump(f);
			if (f->fd != -1)
			{
				logf("[closing old file %ld for '%.11s']\n", f-files, f->filename.bytes);
				close(f->fd);
			}
			f->fd = -1;
			f->filename = *filename;
...
int file_close(cpm_filename_t* filename)
{
	...
	logf("[explicitly closing file %ld for '%.11s']\n", f-files, f->filename.bytes);
	if (f->fd != -1)
	{
...
int file_read(struct file* f, uint8_t* data, uint32_t record)
{
	reopen(f, O_RDONLY);
	
	logf("[read record %08x from file %ld for '%.11s']\n", record, f-files, f->filename.bytes);
	bump(f);
	memset(data, '\0', 128);
	return pread(f->fd, data, 128, record*128);
}

int file_write(struct file* f, uint8_t* data, uint32_t record)
{
	reopen(f, O_RDWR);

	logf("[write record %08x from file %ld for '%.11s']\n", record, f-files, f->filename.bytes);
	bump(f);
...
void file_setrecordcount(struct file* f, uint32_t count)
{
	reopen(f, O_RDONLY);
	
	if (count != file_getrecordcount(f))
	{
		logf("[truncating file %ld to %d records]\n", f-files, count);
		reopen(f, O_RDWR);
